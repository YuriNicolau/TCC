...
stack<CheckpointStruct> forward_saving(f_type *velocity, f_type *damp,
    ...
    //#pragma omp parallel num_threads(NUM_DEVICES) shared (u, velocity, damp, coeff, wavelet, src_points_interval, src_points_values, src_points_values_offset)
    // wavefield modeling
...
extern "C" double gradient(f_type *v, f_type *grad, f_type *velocity, f_type *damp,
    ...
    size_t complete_nz = nz;

    size_t complete_domain_size = complete_nz * nx * ny;

    nz /= NUM_DEVICES;
    size_t domain_size = nz * nx * ny;
    size_t alloc_size = domain_size + 2 * stencil_radius * nx * ny;
    ...
    #pragma omp parallel num_threads(NUM_DEVICES) shared (v, grad, velocity, damp, coeff, wavelet_forward, wavelet_adjoint, src_points_interval, src_points_values, src_points_values_offset, rec_points_interval, rec_points_values, rec_points_values_offset, u)
    {
        ...
        #pragma omp target enter data map(to: velocity[:complete_domain_size]) device(device_id)
        #pragma omp target enter data map(to: damp[:complete_domain_size]) device(device_id)
        #pragma omp target enter data map(to: coeff[:stencil_radius+1]) device(device_id)
        #pragma omp target enter data map(to: src_points_interval[:src_points_interval_size]) device(device_id)
        #pragma omp target enter data map(to: src_points_values[:src_points_values_size]) device(device_id)
        #pragma omp target enter data map(to: src_points_values_offset[:num_sources]) device(device_id)
        #pragma omp target enter data map(to: rec_points_interval[:rec_points_interval_size]) device(device_id)
        #pragma omp target enter data map(to: rec_points_values[:rec_points_values_size]) device(device_id)
        #pragma omp target enter data map(to: rec_points_values_offset[:num_receivers]) device(device_id)
        #pragma omp target enter data map(to: wavelet_forward[:wavelet_forward_size * wavelet_forward_count]) device(device_id)
        #pragma omp target enter data map(to: wavelet_adjoint[:wavelet_adjoint_size * wavelet_adjoint_count]) device(device_id)
        ...
        f_type *snapshot_d_prev = (f_type*) omp_target_alloc(alloc_size * sizeof(f_type), device_id);
        f_type *snapshot_d_current = (f_type*) omp_target_alloc(alloc_size * sizeof(f_type), device_id);

        // copy current checkpoint
        omp_target_memcpy(snapshot_d_prev, snapshots.top().prev, alloc_size * sizeof(f_type), 0, 0, device_id, host);
        omp_target_memcpy(snapshot_d_current, snapshots.top().current, alloc_size * sizeof(f_type), 0, 0, device_id, host);
        ...
        for(size_t n = end_timestep; n >= begin_timestep; n--) {
            ...
            */
            ...
            #pragma omp barrier
            #pragma omp target teams distribute parallel for collapse(3) device(device_id)

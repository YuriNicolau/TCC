...
class SpaceModel:
    ...
    def config_boundary(self, damping_length=0.0, damping_polynomial_degree=3, damping_alpha=0.002):
        """
...
class TimeModel:
    ...
    """
    def __init__(self, space_model, tf, dt=None):
    ...
    def timesteps(self):
        ...
        num_timesteps = int(np.ceil((self.tf + self.dt) / self.dt))
    ...
    def time_values(self):
        ...
        return np.linspace(0, self.tf, self.timesteps,
                           dtype=self.dtype)

    def remove_time_halo_region(self, u):
        """
        Remove the time halo region from the u.
